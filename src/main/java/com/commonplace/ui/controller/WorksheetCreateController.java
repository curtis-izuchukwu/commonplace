...
public class WorksheetCreateController {
    ...
    WorksheetCreateController(
            WorksheetCreationService service, PressWorksheetGenerationService pressService) {
    ...
    private void handleGenerateWithPress() {
        ...
        try {
            request =
                    new PressGenerateRequest(
                            ...
                            readIntegerSpinner(
                                    pressQuestionCountSpinner, 1, 10, "Press question count"),
                            pressFormatCombo.getValue());
        } catch (IllegalArgumentException e) {
            setPressStatus(e.getMessage());
            UiAnimations.validationError(
                    pressSubjectField, pressTopicField, pressQuestionCountSpinner);
            return;
        }
        ...
        Task<PressGenerateResponse> task =
                new Task<>() {
                    @Override
        ...
        task.setOnFailed(
                event -> {
                    ...
                    String message =
                            error == null || textBlank(error.getMessage())
                                    ? "Press generation failed. You can still create the worksheet"
                                            + " manually."
                                    : error.getMessage();
    ...
    private void addQuestionRow(String prompt, String markScheme, int maxMarks, String tags) {
        ...
        Spinner<Integer> maxMarksSpinner =
                new Spinner<>(1, Math.max(100, maxMarks), Math.max(1, maxMarks));
        ...
        ComboBox<DifficultyLevel> questionDifficulty = new ComboBox<>();
        questionDifficulty.getItems().setAll(DifficultyLevel.values());
        questionDifficulty.setValue(difficultyCombo.getValue());
        questionDifficulty.setUserData("assessedDifficulty");
    ...
    private void applyGeneratedWorksheet(
            PressGenerateResponse response, PressGenerateRequest request) {

        if (response.questions().isEmpty()) {
            String message =
                    "Press did not return any usable questions. Manual creation is still"
                            + " available.";
            setPressStatus(message);
            setStatus(message);
            return;
        ...
        if (titleField.getText() == null || titleField.getText().isBlank()) {
            String generatedTitle =
                    response.title().isBlank() ? request.topic() + " Practice" : response.title();
        ...
        questionsContainer
                .getChildren()
                .removeIf(child -> child instanceof VBox row && questionRowIsBlank(row));
        renumberQuestionRows();
                ...
                .forEach(
                        question -> {
                            ...
                            addQuestionRow(
                                    ...
                                    draft.maxMarks(),
                                    draft.tags());
                            VBox generatedRow = (VBox) questionsContainer.getChildren().getLast();
                            generatedRow.getChildren().stream()
                                    .filter(n -> "assessedDifficulty".equals(n.getUserData()))
                                    .forEach(
                                            n -> {
                                                @SuppressWarnings("unchecked")
                                                ComboBox<DifficultyLevel> box =
                                                        (ComboBox<DifficultyLevel>) n;
                                                box.setValue(request.difficulty());
                                            });
                        });
        difficultyCombo.setValue(request.difficulty());
        ...
        String message =
                response.questions().size() != request.questionCount()
                        ? "Press returned "
                                ...
                                + " requested questions. Review before saving."
                        : "Generated "
                                + response.questions().size()
                                + " editable questions. Review before saving.";

        if (response.questions().stream()
                .anyMatch(question -> question.toQuestionDraft().markScheme().isBlank())) {
        ...
        for (int i = 0; i < questionsContainer.getChildren().size(); i++) {
            ...
            for (var child : row.getChildren()) {
                ...
                if (child instanceof TextArea textArea
                        && "markScheme".equals(textArea.getUserData())) {
                    markSchemeArea = textArea;
                }

                if (child instanceof Spinner<?> spinner
                        && "maxMarks".equals(spinner.getUserData())) {
                    @SuppressWarnings("unchecked")
            ...
            SpinnerValueFactory.IntegerSpinnerValueFactory marksFactory =
                    (SpinnerValueFactory.IntegerSpinnerValueFactory)
                            maxMarksSpinner.getValueFactory();
            int maxMarks =
                    readIntegerSpinner(
                            maxMarksSpinner,
                            1,
            ...
            validateQuestionDraftInput(
                    i + 1, promptArea.getText(), markSchemeArea.getText(), maxMarks);
            ...
            drafts.add(
                    new QuestionRepository.QuestionDraft(
                            ...
                            row.getChildren().stream()
                                    .filter(n -> "assessedDifficulty".equals(n.getUserData()))
                                    .map(n -> (DifficultyLevel) ((ComboBox<?>) n).getValue())
                                    .findFirst()
                                    .orElse(difficultyCombo.getValue())));
        }
    ...
    private void validateQuestionDraftInput(
            int questionNumber, String prompt, String markScheme, int maxMarks) {
        ...
        if (markScheme == null || markScheme.isBlank()) {
            throw new IllegalArgumentException(
                    "Question " + questionNumber + " needs a mark scheme.");
        }

        if (maxMarks <= 0) {
            throw new IllegalArgumentException(
                    "Question " + questionNumber + " must have at least 1 mark.");
        }
    }

    private static int readIntegerSpinner(
            Spinner<Integer> spinner, int min, int max, String label) {
        try {
